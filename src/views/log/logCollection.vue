    ...
    methods:{
      formatText(text,index){
        if(text=='评论'){
          ...
          return text+' '+this.data[index].lc.collect_time
        }
        else{
          return text;
        }
      ...
      async addCollection(item,index){
        ...
        this.$confirm('是否确定取消收藏?', '提示', {
          ...
          let result = await http.doPostBean('/deleteLogCollection',params)
          if(result.success){
            this.$message({
              type: 'success',
              message: '取消收藏成功!'
      ...
      async loadData(){
        this.data = await http.doPost('/getLogCollection',{user_id:this.$store.state.user_id})
      ...
      async getActiveActions(){
        ...
        if(this.data.length>=3){
          ...
          for(let i=0;i<this.data.length;i++){
            log_ids.push(this.data[i].log_id)
          }
        }
        let params = {
          log_ids:log_ids,
    ...
    data () {
      return {
        ...
        actions: [
          ...
          { text:'收藏时间',message:'collection_time'},
        ],
      }
    },
    async mounted(){
