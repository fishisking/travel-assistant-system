<template>
    <div>
      <el-page-header @back="goBack" :content="kw+'的搜索结果'">
      </el-page-header>
      <template v-if="field=='0'">
        ...
        <div v-else style="margin-top: 15px;font-size:24px;"><b>搜索结果为空</b></div>
      </template>
      <template v-if="field=='1'">
    ...
    methods: {
      async loadData(){
        if(this.field=='0'){
          ...
          await this.loadHotelData(this.$store.state.city)
        }
      },
      async loadHotelData(val){
        this.hotelData = await http.getHotelDetailByName(this.kw,val);
        console.log(this.hotelData)
      },
      async loadSceneData(){
        this.sceneData = await http.doPost('getSceneByKw',{kw:this.kw})
        console.log(this.hotelData)
      },
